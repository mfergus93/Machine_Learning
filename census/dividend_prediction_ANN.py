...
df_cs=pd.read_excel(path+'\\Census_Supplement.xlsx')
df_cs=(df_cs-df_cs.min())/(df_cs.max()-df_cs.min())
df_rd=pd.read_csv(path+'\\RawData.csv')
...
for node in range(3,7):
     for alph in alphas:
          ...
          plt.xlabel('Epoch')
          plt.savefig(path+'\\Output\\Weight_Plot'+str(node)+'_'+str(alph)+'.png')
          plt.clf()
          ...
          plt.savefig(path+'\\Output\\Loss_Plot'+str(node)+'_'+str(alph)+'.png')
          plt.clf()
...
for node1 in range(1,11):
     for node2 in range(1,5):
          ...
          # plt.savefig(path+'\\Output\\Keras_Plot'+str(node1)+'_'+'str(node2)'+'.png')
          # plt.clf()
